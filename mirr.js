...
if (!user_agent.match("MR_APP/")) {
  throw alert(
    "Please change your user agent to MR_APP/9.30.1/iOS/iPhone11,8/15.0.0"
  );
}
...
if (!account_name) {
  if (mr_id) {
    document.cookie = "mr_id=" + mr_id + "; path=/;";
  }

  throw alert("you need to login first");
...
location.href = "https://mirrativ.com"
}
...
window.onload = () => {
  ...
  async function exec() {
    ...
    const responce = await fetch(
      "https://mirrativ.com/api/gift/update_simple_unique_emomo_gift",
      {
        method: "POST",
        body: form,
      }
    ).then((responce) => responce.text()).catch(alert);
    if (responce.match(/^{/)) {
